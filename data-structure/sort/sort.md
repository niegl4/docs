...
- maxDepth：通过右移位操作实现计算2*ceil(lg(n+1))。限制最大递归深度。
- 排序区间长度<=12,使用【希尔排序】，gap为6。
- 排序区间长度>12，递归最大层数为n，当减小为0时，使用【堆排序】。
              仍然是建立大顶堆（从倒数第一个有子节点的节点开始，自顶向下建堆），再逐个交换堆顶元素与队尾元素并自顶向下堆化。
              巧妙的是，序列的起始索引，并不总是原数组的0号索引。因此，引入序列索引的offset参数。
- 否则使用【快排】。
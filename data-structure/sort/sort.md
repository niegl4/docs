...
最好情况下，数据已经是有序的了，只需要一次冒泡操作，所以最好时间复杂度是O(n)。也即O(1) + O(n)=O(n)。
...
最好情况下，数据已经是有序的了，只需要全部比较一遍，所以最好时间复杂度是O(n)。也即O(n) + O(1)=O(n)。
...
如果数组中的数据原来已经是有序的了，比如 1，3，5，6，8。如果我们每次选择最后一个元素作为 pivot，那每次分区得到的两个区间都是不均等的。我们需要进行大约 n 次分区操作（实际是n-1次，约等于n次），才能完成快排的整个过程。每次分区我们平均要扫描大约 n/2 个元素（实际是 \[(1+n)*n / 2] / n = (1+n)/2，约等于n/2），这种情况下，快排的时间复杂度就从 O(nlogn) 退化成了 O(n^2)。
...
堆排序不是稳定的排序算法，因为在排序的过程，存在将堆的最后一个节点跟堆顶节点互换的操作，所以就有可能改变值相同数据的原始相对顺序。举例：对数组[1, 1]升序，结果原本在前面的1跑后面去了。
